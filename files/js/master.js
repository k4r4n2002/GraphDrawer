...
$(document).ready(function() {
	//give svg element some jquery functions
	hackSVG();
	
	/* Build the Page Html elements */
	...
	// fill the  user option panel
	createUserOptionPanel($("#mainPageInfoPanel"));
	
	// Well Done! page is ready for the user to start playing around
 ...
 * - create new graph btn
 * - upload new graph file btn
 * - save graph as xml file btn
 */
...
/**
 * function to create Drawing buttons
 * - add node btn
 * - node name text
 * - edge weight input
 */
...
//						console.log(event.screenX);
						...
						// Done with draging, Add the node 
						addNodeToGraph({
							x : event.offsetX,
							y : event.offsetY
						});
...
/**
 * fill user options panel with all selectable items and buttons.
 * - includes : the selected graph, from node, to node, required functionality to perform
 * and the evaluate button
 * @param element
 */
function createUserOptionPanel(element) {
	...
	firstPanel.append(selectedGraphHTML).append(otherGraphHTML).append(
			fromToNodesHTML).append(button).append(chcekUncheckAllHTML);

	var SecondPanel = $('<div class="rightUserOption"></div>');

	var isbipartileHTML = $("<div class='row'><input id='isBipartile' type='checkbox' name='userOptions' ><span>Check if Bipartile</span><br>");
	var isPlannerHTML = $("<div class='row'><input id='isPlanner' type='checkbox' name='userOptions' ><span>Check if Planner</span><br>");
	var isTreeHTML = $("<div class='row'><input id='isTree' type='checkbox' name='userOptions'><span>Check if Tree</span><br>");
	var isAllPathesHTML = $("<div class='row'><input id='isListAllPathes' type='checkbox' name='userOptions'><span>List all paths of 1,2,3,... n length</span><br>");
	var isAllCirclesHTML = $("<div class='row'><input id='isListAllCycles'  type='checkbox' name='userOptions'><span>List all cycles (loops) of 3,4,,,,n length</span><br>");
	...
	var isElurHTML = $("<div class='row'><input id='isTestEuler' type='checkbox' name='userOptions'><span>List all Euler paths or cycles</span><br>");
	var isHamiltonHTML = $("<div class='row'><input id='isTestHamilton' type='checkbox' name='userOptions'><span>List all Hamilton paths and cycles</span><br>");
	var isColorGraphHTML = $("<div class='row'><input id='isColorRequired' type='checkbox' name='userOptions'><span>Color the graph</span><br>");
	var isIzomorphismHTML = $("<div class='row'><input id='isIzomorphism' type='checkbox' name='userOptions'><span>Check Izomorphism with other graph</span>");
	var isShortPathHTML = $("<div class='row'><input id='isShortestPath' type='checkbox' name='userOptions'><span>Short Path between Two Nodes</span><br>");
...
/**
 * Create the From Node, To Node selection and set onchange listers 
 */
...
/**
 * listen to changes on fromNode selection
 * @param comboBox
 */
...
/**
 * listen to changes on toNode selection
 * @param comboBox
 */
 ...
 * - the top horizontal info panel
 * - the graph container
 * - the right vertical info panel
 */
...
/**
 * function to check or un-check all checkboxes
 * @param element
 */
...
/**
 * The Evaluate Graph function,
 * to be called when click on Evaluate button.
 * 
 */
function evaluateGraph(graphId) {

	// if not given a graph id, use the open one
	if (!graphId)
		graphId = CUURENT_OPEN_GRAPH;

	var infoPanel = $('#secondPageInfoPanel');
	// clear info panel, from previous run 
	infoPanel.html('');

	// make sure there is a graph opened
	if (graphId == null) {
	...
	// Convert the graph to matrix data form
	...
	if (rows.length > 0) {
		// Show the Degree of Nodes panel
	...
	// Do the required functionalities for each checked combobox
	// one by one
...
function createNewGraph(graphOptions) {
	...
	if (NUM_OF_GRAPHS < 5) {
		...
		// set height of svg
		var minHeight = parseInt(container.css('height')) - 30;
